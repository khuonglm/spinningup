...
def load_policy_and_env(fpath, itr='last', deterministic=False):
    ...
    try:
        state = joblib.load(osp.join(fpath, 'vars'+str(itr)+'.pkl'))
        env = state['env']
    except:
        env = None
...
def load_tf_policy(fpath, itr, deterministic=False):
    ...
    fname = osp.join(fpath, 'tf1_save'+str(itr))
    print('\n\nLoading from %s.\n\n'%fname)
...
if __name__ == '__main__':
    ...
    env, get_action = load_policy_and_env(args.fpath, 
                                          ...
                                          args.deterministic)
    out_file = osp.join(args.fpath, 'model'+str(args.itr)+'.txt')
    run_policy_file(env, get_action, args.len, args.episodes, not(args.norender), out_file)